...
class ConfigurationGuardian:
    def __init__(self):
        ...
        as_ = WorkerASDummy()
        #  WorkerASMoveDistribution()  # WorkerASDummy()
...
class StatMaster:
    def __init__(self, source_dataset_dir, statfile="mock_decoder_stat.csv", image_format=".jpg", is_utc=True):
    ...
    def brand_frame(self, frame, frame_num, kbps):
        ...
        for clr, thcn in zip(color, thickness):
            frame = cv2.putText(frame, text, ps, used_font,
                                font_scale, clr, thcn, cv2.LINE_AA)
        """
        text1 = "Frame {}".format(frame_num)
        text2 = self.get_timestamp()[11:]
        text2 = text2.replace('T', ' ')
        x1 = width // 2 - 14 * len(text1)
        x2 = width // 2 - 14 * len(text2)
        y1 = height // 2 - 25
        y2 = height // 2 + 25
        ps1 = (x1, y1)
        ps2 = (x2, y2)
        ...
        for clr, thcn in zip(color, thickness):
            frame = cv2.putText(frame, text1, ps1, used_font,
                                font_scale, clr, thcn, cv2.LINE_AA)
        for clr, thcn in zip(color, thickness):
            frame = cv2.putText(frame, text2, ps2, used_font,
                                font_scale, clr, thcn, cv2.LINE_AA)
        """
...
def main():
    print("\n=== Инициализация имитатора FPV-CTVP-сервера ===\n")
