...
with open(output, mode='w', encoding='utf-8', newline='') as wf:
    wcsf = csv.writer(wf)
    wcsf.writerow(["name", "ssim", "mse", "psnr", "lpips_vgg", "lpips_alex"])
    ...
    for filer in final_list:
        ...
        lpips_vgg_met, lpips_alex_met = lpips_metric(source_frame, dest_frame)
        ...
        all_lpips_vgg.append(lpips_vgg_met)
        all_lpips_alex.append(lpips_alex_met)

        wcsf.writerow([filer, ssim, mse, psnr, lpips_vgg_met, lpips_alex_met])

all_ssim = np.array(all_ssim)
all_mse = np.array(all_mse)
all_psnr = np.array(all_psnr)
all_lpips_vgg = np.array(all_lpips_vgg)
all_lpips_alex = np.array(all_lpips_alex)

ssim_mean = all_ssim.mean()
mse_mean = all_mse.mean()
psnr_mean = all_psnr.mean()
lpips_vgg_mean = all_lpips_vgg.mean()
lpips_alex_mean = all_lpips_alex.mean()

print("==========")
print("SSIMср = {}".format(ssim_mean))
print("MSEср = {}".format(mse_mean))
print("PSNRср = {} дБ".format(psnr_mean))
print("LPIPS[VGG]ср = {}".format(lpips_vgg_mean))
print("LPIPS[Alex]ср = {}".format(lpips_alex_mean))
print("==========")
