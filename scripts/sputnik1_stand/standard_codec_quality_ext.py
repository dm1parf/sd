...
def lpips_metric(frame1, frame2):
    with torch.no_grad():
...
with open(output, mode='w', encoding='utf-8', newline='') as wf:
    wcsf = csv.writer(wf)
    wcsf.writerow(["enc_img", "dec_img", "ssim", "mse", "psnr", "lpips_vgg", "lpips_alex"])
    ...
    while (cur_enc_frame < (max_encoder_framer + 1)) and (cur_dec_frame < (max_decoder_framer + 1)):
        ...
        mse_met = min_mse
        lpips_vgg_met, lpips_alex_met = lpips_metric(enc_img, dec_img)
        ...
        wcsf.writerow([enc_file, dec_file, ssim_met, mse_met, psnr_met, lpips_vgg_met, lpips_alex_met])
        ...
        print("LPIPS[VGG] = {:.4f}".format(lpips_vgg_met))
        print("LPIPS[Alex] = {:.4f}".format(lpips_alex_met))
...
mean_lpips_vgg = round(sum(lpips_vgg_table) / len(lpips_vgg_table), 4)
mean_lpips_alex = round(sum(lpips_alex_table) / len(lpips_alex_table), 4)
...
print("SSIMср = {:.4f}".format(mean_ssim))
print("LPIPS[VGG]ср = {:.4f}".format(mean_lpips_vgg))
print("LPIPS[Alex]ср = {:.4f}".format(mean_lpips_alex))
