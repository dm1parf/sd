...
#unet = (UNet2DConditionModel.from_pretrained("stabilityai/stable-diffusion-1-4", subfolder="unet").
#        to(device="cuda", dtype=nominal_type))  # use_auth_token=HUGGINGFACE_TOKEN
...
def decoding_test(model, tens):
    image_pixel = model(tens)
    image_pixel = unet(image_pixel, unet_step, None)
    image_restore, _ = worker_restore.restore_work(image_pixel)
    image_restore, _ = worker_sr.sr_work(image_restore, dest_size=[1280, 720])
...
test_model_img = decoding_test(model_true, lpp_tensor)
