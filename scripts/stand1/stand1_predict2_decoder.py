...
if __name__ == "__main__":
    ...
    arguments.add_argument('--forward', dest="forward", type=int, default=6)
...
class FrameManagerProcess(multiprocessing.Process):
    ...
    def __init__(self, actual_list, source_dir, dest_dir="dest_frames", verbose=True, record=False,
                 fps=10, predict_num=1, forward=6, *args, **kwargs):
        super().__init__(*args, **kwargs)
        ...
        self.prediction_buffer_limit = forward + 1
        if self._record:
    ...
    def run(self):
        ...
        while True:
            ...
            if len(self._prediction_buffer) == self.prediction_buffer_limit:
                source_images = [self._prediction_buffer[0], self._prediction_buffer[-1]]
                predict_images, _ = self._predictor.predict_work(source_images, predict_num=self._predict_num)
...
class FPV_CTVP_Server:
    ...
    def __init__(self, source_dir, traceback_mode=False, payload_length=1300, port=6571, actual_length=1,
                 pending_length=10, record=False, fps=30, predict_num=1, forward=6):
        ...
        self._frame_processor = FrameManagerProcess(self._actual_list, self._source_dir, record=self._record,
                                                    fps=self._fps, predict_num=self._predict_num, forward=forward,
                                                    daemon=True)
...
def main():
    ...
    server = FPV_CTVP_Server("dataset_preparation/source_dataset", traceback_mode=True, record=record,
                             payload_length=partition, fps=fps, predict_num=predict_num, forward=forward)
    try:
        server.run_server()
